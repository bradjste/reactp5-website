export default function openSimplexNoise(clientSeed) {
	...
	function createContributionArray(type) {
		...
		const result = createLookupPairs(type.lookup, conts);
		type.base = undefined;
		type.lookup = undefined;
		return result;
	}

	const lookup2D = createContributionArray(types._2D);
	const lookup3D = createContributionArray(types._3D);
	const lookup4D = createContributionArray(types._4D);
